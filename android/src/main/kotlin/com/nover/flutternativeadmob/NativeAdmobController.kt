...
class NativeAdmobController(
    ...
    val listNativeAd: MutableList<UnifiedNativeAd> = mutableListOf()
        ...
        when (CallMethod.valueOf(call.method)) {
            CallMethod.setAdUnitID -> {
                call.argument<String>("adUnitID")?.let {
                    ...
                    if (adLoader == null || isChanged) {
                        this.listNativeAd.clear()
                        val builder = AdLoader.Builder(context, it)
                        adLoader = builder.forUnifiedNativeAd { nativeAd ->
                            this.listNativeAd.add(nativeAd)
                            if(this.nativeAd == null){
                                this.nativeAd = nativeAd
                                invokeLoadCompleted()
                            }else{
                                this.nativeAd = this.listNativeAd.random()
                            }
                    ...
                    if (nativeAd == null || isChanged) loadAd(numberAds) else {
                        invokeLoadCompleted()
                    }
                } ?: result.success(null)
            }
            ...
            CallMethod.reloadAd -> {
                ...
                call.argument<Boolean>("forceRefresh")?.let {
                    ...
                    if (it || nativeAd == null) loadAd(numberAds) else {
                        invokeLoadCompleted()
                    }
                }
            }
    ...
    private fun loadAd(numberAds: Int?) {
        println("loadAd with $numberAds  ${listNativeAd.size}")
